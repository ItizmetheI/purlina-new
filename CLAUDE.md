...
For each sector GLB: load its triangles, rescale/recenter them to roughly the master blob's own scale (skip this and a tiny product collapses every blob vertex onto the same few points instead of tracing its silhouette), build a `MeshBVH` (three-mesh-bvh) over them, then for every master-blob vertex cast a ray toward the product's own center (Blender Shrinkwrap "Project" mode, not "Nearest Surface Point" — nearest-point alone collapses flat/elongated products into a pyramid). Ray origins are pre-scaled by the product's own per-axis bounding half-extents (`computeHalfExtents`) so the projection happens from an ellipsoid matching the product's proportions, not a perfect sphere — without this, flat products like the toy car get an oblique, uneven cast and end up with a "skirt" of collapsed geometry on one side. Falls back to `bvh.closestPointToPoint()` only where a ray doesn't hit anything (concave pockets).

Independent per-vertex projection still leaves a handful of vertices per sector landing on the wrong lobe of the surface relative to their neighbors — fine in isolation, but the triangles touching them stretch into long spikes that read as a crumpled patch in an otherwise clean silhouette. `relaxOutliers()` detects these by comparing each vertex's mean distance to its topological neighbors against the global median, and pulls flagged vertices to their neighbors' average position — leaves correctly-conformed vertices (the large majority) untouched. The result has the sector's silhouette but the blob's exact vertex count/order — that shared topology is what makes the morph possible. Normalize the conformed result (recenter, rescale to unit bounding-sphere radius) so all sectors land at a consistent on-screen scale regardless of the real-world size of the source GLB.
...
Each sector scene also carries a `side` field (`"left"` or `"right"`) in `scroll-script.json` — the text column docks to that side (`.experience__scene--{side}`) and the blob glides to the opposite side, alternating per section for a cinematic, off-center feel rather than a static centered hero. `xSignFor(side)` turns that into `-1`/`0`/`1`; `LiquidBlob.jsx` turns the sign into a target X position scaled by the live viewport width (`state.viewport.width`, not a fixed world-unit offset, so it can't clip off-screen on a narrow/mobile aspect) and eases the mesh toward it every frame via `THREE.MathUtils.damp` — deliberately decoupled from scroll scrub speed so the glide always reads as a smooth, even drift. Sections without a `side` (intro/benefits/outro) keep the blob centered.

Lighting is `<Environment preset="night">` only, no scene lights. "studio"'s HDRI has visible softbox panels in its reflection that read as a product-photography rig — wrong for a "cinematic film" brand feel. "night" gives a dark, moody field with a few sharp pinpoint highlights instead. Direct lights are deliberately absent for the reason in Step 4 below (a metal:1 surface should read reflections from the env map, not a baked-in specular hotspot) — note that sharp/sparse env lighting also makes any residual surface noise (see the Step 2 conform note above) far more visible than soft/broad studio lighting did, since raking specular highlights amplify small normal variation that diffuse light just smooths over. Re-check conform quality under this lighting specifically, not under a softer placeholder env, before judging a sector "clean."
...
- Intro headline/subhead and all 5 sector body copy are draft placeholder text (presentable, not the literal "....." anymore, but not final client copy) — swap for the real Purlina brief copy when available.
...
- Automotive (toy car) conform still has a "skirt" of collapsed geometry under the car body — it's genuinely flat-bottomed, so the projection method has no surface underneath to trace toward; `relaxOutliers()` cleaned up the worst spiking but can't fix the underlying structural gap. Candidate for a real Blender Shrinkwrap export, same as the concave-geometry item above.
- ~~Dark crease/line artifacts~~ — fixed: `smoothNormals()` in `scripts/bake-positions.mjs` blends each vertex's baked normal partway toward its topological neighbors' average (same adjacency-relaxation idea as `relaxOutliers()`, applied to normals instead of positions), softening the genuinely-sharp local dihedral angles that `computeVertexNormals()` was faithfully reporting as creases. Re-baked and confirmed clean on cosmetics' neck band and manufacturing's camera body under the "night" lighting.
